...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == ATK_MB026_UART_INTERFACE)                   /* 如果是ATK-MB026 UART */
    {
        ATK_MB026_UART_TX_GPIO_CLK_ENABLE();                                /* 使能UART TX引脚时钟 */
        ATK_MB026_UART_RX_GPIO_CLK_ENABLE();                                /* 使能UART RX引脚时钟 */
        ATK_MB026_UART_CLK_ENABLE();                                        /* 使能UART时钟 */
        
        gpio_init_struct.Pin        = ATK_MB026_UART_TX_GPIO_PIN;           /* UART TX引脚 */
        gpio_init_struct.Mode       = GPIO_MODE_AF_PP;                      /* 复用推挽输出 */
        gpio_init_struct.Pull       = GPIO_NOPULL;                          /* 无上下拉 */
        gpio_init_struct.Speed      = GPIO_SPEED_FREQ_HIGH;                 /* 高速 */
        HAL_GPIO_Init(ATK_MB026_UART_TX_GPIO_PORT, &gpio_init_struct);      /* 初始化UART TX引脚 */
        
        gpio_init_struct.Pin        = ATK_MB026_UART_RX_GPIO_PIN;           /* UART RX引脚 */
        gpio_init_struct.Mode       = GPIO_MODE_INPUT;                      /* 复用推挽输出 */
        gpio_init_struct.Pull       = GPIO_NOPULL;                          /* 无上下拉 */
        gpio_init_struct.Speed      = GPIO_SPEED_FREQ_HIGH;                 /* 高速 */
        HAL_GPIO_Init(ATK_MB026_UART_RX_GPIO_PORT, &gpio_init_struct);      /* 初始化UART RX引脚 */
        
        HAL_NVIC_SetPriority(ATK_MB026_UART_IRQn, 0, 0);                    /* 抢占优先级0，子优先级0 */
        HAL_NVIC_EnableIRQ(ATK_MB026_UART_IRQn);                            /* 使能UART中断通道 */
